...
const SavePath: FC<{ state: State }> = (props) => {
  ...
  useMount(async () => {
    ...
    try {
      setCacheDir(await appCacheDir());
    } catch {
      setCacheDir(await tauriDataDir());
    }
  });
      ...
      >
        <Space.Compact>
          <Tooltip title="自定义">
            <Button
              icon={<NodeIndexOutlined />}
              onClick={() => handlePickPath(setLogDir)}
            />
          </Tooltip>
          <Tooltip title="恢复默认">
            <Button
              icon={<ReloadOutlined />}
              onClick={async () => setLogDir(await appLogDir())}
            />
          </Tooltip>
        </Space.Compact>
      </ProListItem>
        ...
        <Space.Compact>
          ...
          <Tooltip title="恢复默认">
            <Button
              icon={<ReloadOutlined />}
              onClick={async () => {
                try { setCacheDir(await appCacheDir()); } catch { setCacheDir(await tauriDataDir()); }
              }}
            />
          </Tooltip>
        </Space.Compact>
