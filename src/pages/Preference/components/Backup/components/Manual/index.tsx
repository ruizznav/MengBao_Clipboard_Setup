...
import { Flex, List, message, Modal } from "antd";
...
const Manual: FC<{ state: State }> = (props) => {
  ...
  const handleImport = async () => {
    const confirmed = await new Promise<boolean>((resolve) => {
      Modal.confirm({
        ...
        okText: t("preference.data_backup.import_export.button.confirm_import"),
  ...
  const handleExport = async () => {
    const confirmed = await new Promise<boolean>((resolve) => {
      Modal.confirm({
        ...
        content: t("preference.data_backup.import_export.hints.confirm_export"),
        okText: t("preference.data_backup.import_export.button.confirm_export"),
        cancelText: t("preference.data_backup.import_export.button.cancel"),
        okButtonProps: { danger: true },
        onOk: () => resolve(true),
        onCancel: () => resolve(false),
    ...
    try {
      ...
      const defaultPath = join(await downloadDir(), `${filename}.${extname()}`);
  ...
  const mapList = [
    {
      event: handleImport,
      icon: "i-hugeicons:database-restore",
